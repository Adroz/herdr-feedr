...
**Status: core + CLI + sidebar TUI implemented** (`cargo install --path .` → `feedr sidebar`, or `feedr sidebar --dock` inside herdr). Herdr plugin packaging and the agent skill are next; the v1 spec is in [docs/SPEC.md](docs/SPEC.md); it was decided through the wayfinder map in [issue #1](https://github.com/Adroz/herdr-feedr/issues/1) (research findings live on `research/*` branches, format prototypes on `prototype/feed-format`).
